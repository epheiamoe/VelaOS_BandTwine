...
- ⚡ **动作系统**：set/add/toggle/vibrate/toast/jump 等多种动作类型
- 🎲 **随机系统**：权重、条件随机选项
- 🖼️ **图片系统**：图片定义管理
- 🔀 **条件系统**：条件分支选项
- 📁 **导入导出**：支持 BandTwine JSON 格式和完整项目文件
- 🧪 **测试覆盖**：使用 Vitest 和 Vue Testing Library 进行 TDD 开发
- 🚀 **实时预览**：文本编译预览（变量、条件、随机、图片标记）
- 💾 **变量管理**：show/temp 变量分类支持
...
1. ✅ **Vue Flow 完全集成**：已实现拖拽、缩放、连线
2. ✅ **编译脚本集成**：实时预览编译结果（变量替换、标记预览）
3. ✅ **高级功能**：条件系统、随机选项、动作系统（已完成）
